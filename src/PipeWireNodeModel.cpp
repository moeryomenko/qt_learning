...
static void on_global(void* data, uint32_t id, uint32_t /*perms*/, const char* type,
  ...
  qDebug() << "[PipeWireNodeModel] on_global id:" << id << "mediaClass:" << mc << "name:" << n.name;

  // Marshal to Qt main thread
  QMetaObject::invokeMethod(self, [self, n]() { self->addOrUpdate(n); }, Qt::QueuedConnection);
}
...
static void on_global_remove(void* data, uint32_t id) {
  ...
  QMetaObject::invokeMethod(self, [self, id]() { self->removeNode(id); }, Qt::QueuedConnection);
}
...
PipeWireNodeModel::PipeWireNodeModel(QObject* parent) : QAbstractListModel(parent) {
...
QHash<int, QByteArray> PipeWireNodeModel::roleNames() const {
  return {
      {IdRole, "nodeId"},       {NameRole, "name"},         {MediaClassRole, "mediaClass"},
      {AppNameRole, "appName"}, {NodeNickRole, "nodeNick"}, {IsMonitorRole, "isMonitor"},
      {IsAudioRole, "isAudio"}, {IsVideoRole, "isVideo"},   {IsStreamRole, "isStream"},
  };
}
