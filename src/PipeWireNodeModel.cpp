...
static void on_global(void *data, uint32_t id, uint32_t /*perms*/,
  ...
  qDebug() << "[PipeWireNodeModel] on_global id:" << id << "mediaClass:" << mc
           << "name:" << n.name;

  // Marshal to Qt main thread
  QMetaObject::invokeMethod(
      self, [self, n]() { self->addOrUpdate(n); }, Qt::QueuedConnection);
}
...
static void on_global_remove(void *data, uint32_t id) {
  ...
  QMetaObject::invokeMethod(
      self, [self, id]() { self->removeNode(id); }, Qt::QueuedConnection);
}
...
void PipeWireNodeModel::addOrUpdate(const Node &n) {
  ...
  m_nodes.push_back(n);
  endInsertRows();
  qDebug() << Q_FUNC_INFO << "Node added id:" << n.id
           << "class:" << n.mediaClass;
}
