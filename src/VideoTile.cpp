...
VideoTile::VideoTile(QQuickItem* parent)
...
void VideoTile::onFrame(const QImage& frame) {
    ...
    if (!m_hasVideo) setHasVideo(true);
...
void VideoTile::paint(QPainter* painter) {
    ...
    if (!frame.isNull() && m_videoEnabled) {
        ...
        const QRectF dest(
            bounds.x() + (bounds.width()  - frameSize.width())  / 2.0,
