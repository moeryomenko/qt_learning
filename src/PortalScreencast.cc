...
static constexpr const char* SCREENCAST_IF = "org.freedesktop.portal.ScreenCast";
...
void PortalScreencast::stop() {
  ...
  if (!m_sessionHandle.isEmpty()) {
    QDBusMessage msg =
        QDBusMessage::createMethodCall(PORTAL_BUS, PORTAL_PATH, SCREENCAST_IF, "CloseSession");
...
void PortalScreencast::createSession() {
  ...
  QDBusMessage msg =
      QDBusMessage::createMethodCall(PORTAL_BUS, PORTAL_PATH, SCREENCAST_IF, "CreateSession");

  QVariantMap opts;
  opts["handle_token"]         = QString("qt_%1").arg(QRandomGenerator::global()->generate());
  opts["session_handle_token"] = QString("sess_%1").arg(QRandomGenerator::global()->generate());
  msg << opts;

  QDBusPendingReply<QDBusObjectPath> reply = QDBusConnection::sessionBus().asyncCall(msg);
  ...
  connect(watcher, &QDBusPendingCallWatcher::finished, this,
          [this, watcher](QDBusPendingCallWatcher* w) {
            ...
            qDebug() << Q_FUNC_INFO << "Awaiting CreateSession response on:" << reqPath;
...
void PortalScreencast::handleCreateSessionResponse(uint response, const QVariantMap& results) {
...
void PortalScreencast::selectSources() {
  ...
  QDBusMessage msg =
      QDBusMessage::createMethodCall(PORTAL_BUS, PORTAL_PATH, SCREENCAST_IF, "SelectSources");

  QVariantMap opts;
  opts["handle_token"] = QString("sel_%1").arg(QRandomGenerator::global()->generate());
  ...
  connect(watcher, &QDBusPendingCallWatcher::finished, this,
          [this, watcher](QDBusPendingCallWatcher* w) {
            ...
            const QString reqPath = r.value().path();
            qDebug() << Q_FUNC_INFO << "Awaiting SelectSources response on:" << reqPath;
            QDBusConnection::sessionBus().connect(
                PORTAL_BUS, reqPath, REQUEST_IF, "Response", this,
...
void PortalScreencast::startSession() {
  ...
  QDBusMessage msg =
      QDBusMessage::createMethodCall(PORTAL_BUS, PORTAL_PATH, SCREENCAST_IF, "Start");

  QVariantMap opts;
  opts["handle_token"] = QString("start_%1").arg(QRandomGenerator::global()->generate());
  ...
  connect(watcher, &QDBusPendingCallWatcher::finished, this,
          [this, watcher](QDBusPendingCallWatcher* w) {
            ...
            QDBusConnection::sessionBus().connect(PORTAL_BUS, reqPath, REQUEST_IF, "Response", this,
                                                  SLOT(handleStartResponse(uint, QVariantMap)));
          });
}

void PortalScreencast::handleStartResponse(uint response, const QVariantMap& results) {
  qDebug() << Q_FUNC_INFO << "response:" << response;
  if (response != 0) {
  ...
  qDebug() << Q_FUNC_INFO << "[FIX] streams variant type:" << streamsVariant.typeName()
  ...
  if (streamsVariant.canConvert<QDBusArgument>()) {
    ...
    arg.endArray();
    qDebug() << Q_FUNC_INFO << "[FIX] Parsed" << streamCount << "streams via QDBusArgument";
  } else {
...
void PortalScreencast::openPipeWireRemote() {
  qDebug() << Q_FUNC_INFO << "Opening PipeWire remote for session:" << m_sessionHandle;
  QDBusMessage msg =
      QDBusMessage::createMethodCall(PORTAL_BUS, PORTAL_PATH, SCREENCAST_IF, "OpenPipeWireRemote");

  QVariantMap opts;
  opts["handle_token"] = QString("pw_%1").arg(QRandomGenerator::global()->generate());
  msg << QDBusObjectPath(m_sessionHandle) << opts;

  QDBusPendingReply<QDBusUnixFileDescriptor> reply = QDBusConnection::sessionBus().asyncCall(msg);
  ...
  connect(watcher, &QDBusPendingCallWatcher::finished, this,
          [this, watcher](QDBusPendingCallWatcher* w) {
            ...
            m_active = true;
            emit activeChanged();
            emit info("Screen cast active. videoNodeId=" + QString::number(m_videoNodeId)
                      + " fd=" + QString::number(m_pipeWireFd));
            qDebug() << Q_FUNC_INFO << "PortalScreencast active. fd:" << m_pipeWireFd
