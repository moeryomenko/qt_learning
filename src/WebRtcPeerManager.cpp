...
gboolean WebRtcPeerManager::busCallback(GstBus*, GstMessage* msg, gpointer user_data) {
    ...
    if (GST_MESSAGE_TYPE(msg) == GST_MESSAGE_ERROR) {
        ...
        if (GST_MESSAGE_SRC(msg) == GST_OBJECT(self->m_pipeline)) {
            ...
            qDebug() << "[WebRtcPeerManager] Pipeline state:"
                     << gst_element_state_get_name(old_) << "->"
...
WebRtcPeerManager::WebRtcPeerManager(QObject* parent)
    : QObject(parent)
{
...
bool WebRtcPeerManager::startWithCamera() {
    ...
    qDebug() << Q_FUNC_INFO << "[FIX] Saved" << savedPeers.size()
             << "peers and" << savedOffers.size() << "offers for reconnect";
    stop();
...
bool WebRtcPeerManager::startScreenShare() {
    ...
    QObject::connect(m_videoSink, &QVideoSink::videoFrameChanged,
                     this, &WebRtcPeerManager::onVideoFrame,
                     Qt::DirectConnection);
...
void WebRtcPeerManager::onRemoteOffer(const QString& peerId, const QString& sdp) {
    ...
    // Create answer
    struct Ctx { WebRtcPeerManager* self; QString peerId; GstElement* webrtcbin; };
    auto* ctx = new Ctx{this, peerId, p.webrtcbin};

    GstPromise* answerPromise = gst_promise_new_with_change_func(
        &WebRtcPeerManager::onAnswerCreated, ctx,
        [](gpointer p){ delete static_cast<Ctx*>(p); });
...
void WebRtcPeerManager::onRemoteIce(const QString& peerId, const IceCandidate& ice) {
    qDebug() << Q_FUNC_INFO << "Remote ICE from:" << peerId
             << "sdpMid:" << ice.sdpMid << "mline:" << ice.sdpMLineIndex;
    if (!m_peers.contains(peerId)) {
    ...
    g_signal_emit_by_name(p.webrtcbin, "add-ice-candidate",
                          (guint)ice.sdpMLineIndex, c.constData());
}

// ---------- Qt Multimedia frame callback -------------------------------------

void WebRtcPeerManager::onVideoFrame(const QVideoFrame& frame) {
    if (!m_appsrc || !m_running || !frame.isValid()) return;
...
bool WebRtcPeerManager::buildPipeline() {
    ...
    // later with a cryptic "Failed to create webrtcbin" message.
    {
        ...
        if (!wbf) {
            ...
            emit error("WebRTC requires gst-plugins-bad — Arch: sudo pacman -S gst-plugins-bad\n"
    ...
    if (m_videoSource == VideoSourceMode::Camera) {
        ...
        if (!vsrc) vsrc = gst_element_factory_make("v4l2src", "vsrc");
        if (!vsrc) {
            qWarning() << Q_FUNC_INFO << "[FIX] autovideosrc/v4l2src not found — no camera source available";
        ...
        const QByteArray capsStr = QString("video/x-raw,format=RGBA,width=%1,height=%2,framerate=0/1")
                                       .arg(w).arg(h).toUtf8();
        ...
        qDebug() << Q_FUNC_INFO << "[FIX] appsrc configured:" << capsStr
                 << "screen" << w << "x" << h;
    }
    ...
    if (lpreview) gst_object_ref(lpreview); // pipeline will own a ref
    ...
    if (!asrc) asrc   = gst_element_factory_make("pipewiresrc",    "asrc");
    ...
    {
        struct { const char* name; const char* pkg; GstElement* el; } checks[] = {
            ...
            { "opusenc",       "gst-plugins-base (NOTE: gst-plugins-base-libs is NOT enough) — Arch: sudo pacman -S gst-plugins-base; verify: gst-inspect-1.0 opusenc", aenc },
            { "rtpopuspay",    "gst-plugins-good (RTP Opus payloader) — Arch: sudo pacman -S gst-plugins-good; verify: gst-inspect-1.0 rtpopuspay", apay },
        ...
        for (auto& c : checks) {
            if (!c.el) {
                qWarning() << Q_FUNC_INFO << "[FIX] Missing element:" << c.name
                           << "— install:" << c.pkg;
                missing = true;
            }
        }
    ...
    if (lpreview) {
        gst_bin_add_many(GST_BIN(m_pipeline),
                         vsrc, vconv, vscale, vcaps, m_videoValve, venc, vpay, m_vtee,
                         asrc, aconv, ares, acaps, m_audioValve, aenc, apay, m_atee,
                         lpreview,
                         nullptr);
    } else {
        gst_bin_add_many(GST_BIN(m_pipeline),
    ...
    // Link video chain
    if (!gst_element_link_many(vsrc, vconv, vscale, vcaps, m_videoValve, venc, vpay, m_vtee, nullptr)) {
    ...
    if (lpreview) {
        ...
        if (teeSrc) gst_object_unref(teeSrc);
...
bool WebRtcPeerManager::addWebRtcBinForPeer(const QString& peerId) {
    ...
    GstElement* wb = gst_element_factory_make("webrtcbin",
                                              ("wb_" + peerId).toUtf8().constData());
    if (!wb) {
    ...
    if (sink->element()) {
        gst_object_ref(sink->element()); // VideoFrameSink keeps this ref; pipeline gets its own via gst_bin_add
    }
    gst_bin_add(GST_BIN(m_pipeline), sink->element());
    gst_element_sync_state_with_parent(sink->element());
    qDebug() << Q_FUNC_INFO << "[FIX] Peer appsink added with extra ref — refcount balanced for teardown";
    ...
    g_signal_connect(wb, "on-negotiation-needed",
                     G_CALLBACK(WebRtcPeerManager::onNegotiationNeeded), this);

    // Connect ICE candidate
    g_signal_connect(wb, "on-ice-candidate",
                     G_CALLBACK(WebRtcPeerManager::onIceCandidate), this);
    ...
    if (m_pendingIce.contains(peerId)) {
        ...
        for (const IceCandidate& ice : pending) {
            const QByteArray c = ice.candidate.toUtf8();
            g_signal_emit_by_name(addedPeer.webrtcbin, "add-ice-candidate",
                                  (guint)ice.sdpMLineIndex, c.constData());
        }
    }
...
bool WebRtcPeerManager::linkTeeToWebrtcBin(GstElement* tee, GstElement* webrtcbin,
    ...
    if (!qSrc || !wbSink || gst_pad_link(qSrc, wbSink) != GST_PAD_LINK_OK) {
        ...
        if (wbSink) gst_object_unref(wbSink);
        return false;
    }
...
QString WebRtcPeerManager::extractSdpMid(GstElement* webrtcbin, int mlineIndex) const {
    ...
    if ((guint)mlineIndex < count) {
        ...
        if (val) mid = QString::fromUtf8(val);
    }

    gst_webrtc_session_description_free(localDesc);

    if (mid.isEmpty()) mid = QString::number(mlineIndex);
...
WebRtcPeerManager::Peer* WebRtcPeerManager::peerByWebrtcBin(GstElement* bin) {
    for (auto it = m_peers.begin(); it != m_peers.end(); ++it) {
        if (it->webrtcbin == bin) return &it.value();
    }
    return nullptr;
}
...
void WebRtcPeerManager::onNegotiationNeeded(GstElement* webrtcbin, gpointer user_data) {
    ...
    struct Ctx { WebRtcPeerManager* self; QString peerId; GstElement* wb; };
    auto* ctx = new Ctx{self, p->id, webrtcbin};

    GstPromise* promise = gst_promise_new_with_change_func(
        &WebRtcPeerManager::onOfferCreated, ctx,
        [](gpointer p){ delete static_cast<Ctx*>(p); });
...
void WebRtcPeerManager::onOfferCreated(GstPromise* promise, gpointer user_data) {
    ...
    QMetaObject::invokeMethod(ctx->self, [self = ctx->self, peerId, sdp]() {
        emit self->localOfferReady(peerId, sdp);
    }, Qt::QueuedConnection);
}
...
void WebRtcPeerManager::onAnswerCreated(GstPromise* promise, gpointer user_data) {
    ...
    QMetaObject::invokeMethod(ctx->self, [self = ctx->self, peerId, sdp]() {
        emit self->localAnswerReady(peerId, sdp);
    }, Qt::QueuedConnection);
}

void WebRtcPeerManager::onIceCandidate(GstElement* webrtcbin, guint mline,
    ...
    qDebug() << "[WebRtcPeerManager] ICE candidate for peer:" << p->id
             << "sdpMid:" << sdpMid << "mline:" << mline;

    const QString peerId = p->id;
    QMetaObject::invokeMethod(self, [self, peerId, ice]() {
        emit self->localIceReady(peerId, ice);
    }, Qt::QueuedConnection);
}
...
void WebRtcPeerManager::onPadAdded(GstElement* webrtcbin, GstPad* pad, gpointer user_data) {
    ...
    if (!decodebin || !vconv2 || !vscale2 || !appsinkEl) {
        qWarning() << "[WebRtcPeerManager] Missing decode elements"
                   << decodebin << vconv2 << vscale2 << appsinkEl;
        if (decodebin) gst_object_unref(decodebin);
        if (vconv2) gst_object_unref(vconv2);
        if (vscale2) gst_object_unref(vscale2);
        return;
    }
    ...
    struct DecodeCtx { GstElement* vconv; GstElement* vscale; GstElement* appsink; };
    auto* ctx2 = new DecodeCtx{vconv2, vscale2, appsinkEl};

    g_signal_connect(decodebin, "pad-added",
                     G_CALLBACK(+[](GstElement*, GstPad* decodePad, gpointer ud) {
        auto* c = static_cast<DecodeCtx*>(ud);

        GstCaps* pcaps = gst_pad_get_current_caps(decodePad);
        if (!pcaps) pcaps = gst_pad_query_caps(decodePad, nullptr);
        bool isVideo = false;
        if (pcaps) {
            isVideo = g_str_has_prefix(
                gst_structure_get_name(gst_caps_get_structure(pcaps, 0)), "video/");
    ...
    qDebug() << "[WebRtcPeerManager] Receive pipeline connected for peer:"
             << (p ? p->id : "unknown");
}

void WebRtcPeerManager::onIceConnectionStateChange(GObject* webrtcbin,
                                                   GParamSpec* /*pspec*/,
                                                   gpointer user_data)
{
    ...
    switch (state) {
        case GST_WEBRTC_ICE_CONNECTION_STATE_NEW:          stateStr = "new";          break;
        case GST_WEBRTC_ICE_CONNECTION_STATE_CHECKING:     stateStr = "checking";     break;
        case GST_WEBRTC_ICE_CONNECTION_STATE_CONNECTED:    stateStr = "connected";    break;
        case GST_WEBRTC_ICE_CONNECTION_STATE_COMPLETED:    stateStr = "completed";    break;
        case GST_WEBRTC_ICE_CONNECTION_STATE_FAILED:       stateStr = "FAILED";       break;
        case GST_WEBRTC_ICE_CONNECTION_STATE_DISCONNECTED: stateStr = "disconnected"; break;
        case GST_WEBRTC_ICE_CONNECTION_STATE_CLOSED:       stateStr = "closed";       break;
        default: break;
    }

    QString stateQStr = QString::fromUtf8(stateStr);
    QMetaObject::invokeMethod(self, [self, peerId, stateQStr]() {
        qDebug() << "[WebRtcPeerManager] ICE connection state →" << stateQStr
                 << "peer:" << peerId;
        if (stateQStr == "FAILED") {
...
void WebRtcPeerManager::onConnectionStateChange(GObject* webrtcbin,
                                                GParamSpec* /*pspec*/,
                                                gpointer user_data)
{
    auto* self = static_cast<WebRtcPeerManager*>(user_data);
