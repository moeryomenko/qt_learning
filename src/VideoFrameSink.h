...
// Lives on the Qt main thread; the on_new_sample callback runs on the GStreamer thread
// and posts frameReady via QMetaObject::invokeMethod(Qt::QueuedConnection).
...
public:
    ...
    // Call this ONCE after construction to get the GstElement* to add to a pipeline.
    // Ownership stays with this object (ref held via gst_object_ref).
