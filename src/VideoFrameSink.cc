...
VideoFrameSink::VideoFrameSink(QObject* parent)
  ...
  g_object_set(m_appsink, "emit-signals", TRUE, "caps", caps, "max-buffers",
               1,  // drop old frames if Qt thread is slow
...
GstFlowReturn VideoFrameSink::on_new_sample(GstAppSink* sink, gpointer user_data) {
  ...
  QMetaObject::invokeMethod(
      self, [self, frame = std::move(frame)]() mutable { emit self->frameReady(std::move(frame)); },
      Qt::QueuedConnection);
