    ...
    : QObject(parent)
{
    ...
    connect(&m_ws, &QWebSocket::sslErrors,
...
void SignalingClient::onReconnectTimeout() {
    ...
    if (!m_token.isEmpty()) url += "?token=" + m_token;
    m_ws.open(QUrl(url));
}
...
void SignalingClient::scheduleReconnect() {
    ...
    if (m_reconnectAttempts >= kMaxReconnectAttempts) return;

    // Exponential backoff: 1s, 2s, 4s, 8s, 16s (capped at 30s)
    const int delayMs = qMin(kBaseReconnectMs * static_cast<int>(std::pow(2, m_reconnectAttempts)), 30000);
    m_reconnectAttempts++;
