...
SignalingClient::SignalingClient(QObject *parent) : QObject(parent) {
  ...
  connect(&m_reconnectTimer, &QTimer::timeout, this,
          &SignalingClient::onReconnectTimeout);
}

void SignalingClient::connectToServer(const QString &wsBaseUrl,
                                      const QString &token) {
  qDebug() << Q_FUNC_INFO << "Connecting to" << wsBaseUrl
           << "with token empty:" << token.isEmpty();
...
void SignalingClient::join(const QString &roomId, const QString &username) {
  ...
  if (!m_connected) {
    qWarning() << Q_FUNC_INFO
               << "Not connected — join will be sent after connection";
    return;
  }
...
void SignalingClient::sendOffer(const QString &toPeerId, const QString &sdp) {
  qDebug() << Q_FUNC_INFO << "Sending offer to peer:" << toPeerId
...
void SignalingClient::sendAnswer(const QString &toPeerId, const QString &sdp) {
  qDebug() << Q_FUNC_INFO << "Sending answer to peer:" << toPeerId
           << "sdp length:" << sdp.length();
  QJsonObject payload;
  payload["sdp"] = sdp;
  payload["type"] = "answer";
...
void SignalingClient::sendIce(const QString &toPeerId,
                              const IceCandidate &ice) {
  qDebug() << Q_FUNC_INFO << "Sending ICE to peer:" << toPeerId
...
void SignalingClient::onDisconnected() {
  qDebug() << Q_FUNC_INFO
           << "WebSocket disconnected. Was in room:" << m_currentRoom;
  m_connected = false;
  emit connectedChanged();
...
void SignalingClient::onReconnectTimeout() {
  if (m_reconnectAttempts >= kMaxReconnectAttempts) {
    ...
    setError("Connection lost after " + QString::number(kMaxReconnectAttempts) +
             " retries");
    return;
  }
...
void SignalingClient::sendJson(const QJsonObject &obj) {
  if (!m_connected) {
    qWarning() << Q_FUNC_INFO
               << "Cannot send, not connected. type:" << obj["type"].toString();
    return;
  }
  const QString text =
...
void SignalingClient::scheduleReconnect() {
  ...
  const int delayMs = qMin(
      kBaseReconnectMs * static_cast<int>(std::pow(2, m_reconnectAttempts)),
      30000);
  m_reconnectAttempts++;
...
void SignalingClient::handleMessage(const QJsonObject &obj) {
  ...
  qDebug() << Q_FUNC_INFO << "Handling message type:" << type
           << "from:" << from;
  ...
  if (type == "joined") {
    ...
    for (auto it = roomMates.begin(); it != roomMates.end(); ++it) {
      ...
      qDebug() << Q_FUNC_INFO << "Existing peer:" << peerId
               << "name:" << peerName;
    }
    emit peersChanged();
  ...
  if (type == "peer-joined") {
    qDebug() << Q_FUNC_INFO << "Peer joined:" << from
             << "username:" << username;
  ...
  if (type == "offer") {
    ...
    qDebug() << Q_FUNC_INFO << "Received offer from:" << from
             << "sdp length:" << sdp.length();
  ...
  if (type == "answer") {
    const QString sdp = payload.value("sdp").toString();
    qDebug() << Q_FUNC_INFO << "Received answer from:" << from
             << "sdp length:" << sdp.length();
  ...
  if (type == "ice") {
    ...
    qDebug() << Q_FUNC_INFO << "Received ICE from:" << from
             << "sdpMid:" << ice.sdpMid << "mline:" << ice.sdpMLineIndex;
