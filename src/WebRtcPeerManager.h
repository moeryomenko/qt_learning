...
class WebRtcPeerManager : public QObject {
  ...
  Q_PROPERTY(bool cameraMuted READ cameraMuted WRITE setCameraMuted NOTIFY
                 cameraMutedChanged)
  Q_PROPERTY(
      VideoSourceMode videoSource READ videoSource NOTIFY videoSourceChanged)
...
public:
  ...
  bool cameraMuted() const { return m_cameraMuted; }
  Q_INVOKABLE void setCameraMuted(bool m);

  VideoSourceMode videoSource() const { return m_videoSource; }
  ...
  VideoFrameSink *localSink() const { return m_localSink; }
...
private:
  ...
  bool linkTeeToWebrtcBin(GstElement *tee, GstElement *webrtcbin,
                          const QString &label);
  ...
  static void onIceCandidate(GstElement *webrtcbin, guint mline,
  ...
  static void onPadAdded(GstElement *webrtcbin, GstPad *pad,
                         gpointer user_data);
  // notify::ice-connection-state / notify::connection-state — GObject property
  // notifications
  static void onIceConnectionStateChange(GObject *webrtcbin, GParamSpec *pspec,
                                         gpointer user_data);
  static void onConnectionStateChange(GObject *webrtcbin, GParamSpec *pspec,
                                      gpointer user_data);
