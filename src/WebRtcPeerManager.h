...
private:
    ...
    // notify::ice-connection-state / notify::connection-state — GObject property notifications
...
private:
    ...
    // Portal screen share params (used when m_videoSource == ScreenShare via portal)
    int  m_portalFd    = -1;
    ...
    // Remote offers that arrived before the pipeline was running (buffered for later processing)
